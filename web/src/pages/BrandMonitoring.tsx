...
export default function BrandMonitoring() {
  ...
  return (
    <motion.div className="qa-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: 32 }}>
      ...
      <p style={{ color: "var(--text-secondary)", marginBottom: 16 }}>Monitor brand presence, sentiment, and visibility across crawled pages using AI analysis.</p>
      ...
      <div className="qa-panel" style={{ padding: 16, marginTop: 12, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button className="qa-btn" onClick={analyze} disabled={loading || !runId || !brandName.trim()} style={{ padding: "8px 24px" }}>{loading ? "Analyzing..." : "Monitor Brand"}</button>
      </div>

      {error && <div className="qa-panel" style={{ marginTop: 16, color: "#e53e3e", padding: 16 }}>{error}</div>}
      {loading && <div className="qa-panel" style={{ marginTop: 20, textAlign: "center", padding: 40 }}>Monitoring brand presence...</div>}
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 16, marginTop: 16, flexWrap: "wrap" }}>
            {sentData.length > 0 && (
              <div className="qa-panel" style={{ padding: 16, width: 260 }}>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 8 }}>Sentiment</div>
            ...
            {data.brandStrength && (
              <div className="qa-panel" style={{ padding: 16, flex: 1 }}>
                <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Brand Strength</div>
          ...
          {(data.alerts ?? []).length > 0 && (
            <div className="qa-panel" style={{ marginTop: 16, padding: 16, borderLeft: "3px solid #e53e3e" }}>
              <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 8, color: "#e53e3e" }}>Alerts</div>
