...
export default function PostTracking() {
  ...
  return (
    <motion.div className="qa-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: 32 }}>
      ...
      <p style={{ color: "var(--text-secondary)", marginBottom: 16 }}>Track content changes and performance across crawl runs. Compare current vs. baseline to detect modifications.</p>
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginBottom: 12 }}>
        ...
        <button className="qa-btn" onClick={track} disabled={loading || !runId} style={{ padding: "8px 24px", alignSelf: "flex-end" }}>{loading ? "Tracking..." : "Track Changes"}</button>
      </div>

      {error && <div className="qa-panel" style={{ marginTop: 16, color: "#e53e3e" }}>{error}</div>}
      {loading && <div className="qa-panel" style={{ marginTop: 20, textAlign: "center", padding: 40 }}>Tracking posts...</div>}
      ...
      {data && !loading && (
        <>
          ...
          {!trends.hasBaseline && (
            <div className="qa-panel" style={{ marginTop: 16, padding: 16, color: "var(--text-secondary)", textAlign: "center" }}>No baseline run selected. Select a baseline to see content changes between runs.</div>
          )}

          {changes.length > 0 && (
            <div className="qa-panel" style={{ marginTop: 16, padding: 16, overflowX: "auto" }}>
              <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Content Changes ({changes.length})</div>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr>{["URL", "Status", "Change Type", "Performance"].map(h => <th key={h} style={{ padding: "8px 10px", textAlign: h === "URL" ? "left" : "center", fontSize: 12, color: "var(--text-secondary)", borderBottom: "2px solid var(--border)" }}>{h}</th>)}</tr></thead>
