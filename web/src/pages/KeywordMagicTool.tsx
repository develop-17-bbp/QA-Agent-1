...
export default function KeywordMagicTool() {
  ...
  return (
    <motion.div className="qa-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: 32 }}>
      ...
      <p style={{ color: "var(--text-secondary)", marginBottom: 16 }}>Enter a seed keyword to discover related keywords, search volumes, and clusters powered by Gemini AI.</p>
      ...
      {loading && <div className="qa-panel" style={{ marginTop: 20, textAlign: "center", padding: 40 }}>Generating keyword ideas...</div>}
        ...
        <>
          ...
          <div className="qa-panel" style={{ marginTop: 16, padding: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ fontSize: 14, fontWeight: 600 }}>Keywords ({filtered.length})</div>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead><tr>
                  {["Keyword", "Volume", "Difficulty", "Intent", "CPC", "Trend"].map(h => (
                    <th key={h} style={{ padding: "8px 10px", textAlign: "left", fontSize: 12, color: "var(--text-secondary)", borderBottom: "2px solid var(--border)" }}>{h}</th>
                  ))}
                </tr></thead>
                <tbody>
                  {filtered.map((kw: any, i: number) => (
                    <tr key={i} style={{ borderBottom: "1px solid var(--border)" }}>
                      ...
                      <td style={{ padding: "6px 10px", fontSize: 13, color: "var(--text-secondary)" }}>{kw.volume}</td>
                      <td style={{ padding: "6px 10px" }}><span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, background: (DIFF_COLORS[kw.difficulty] ?? "#888") + "20", color: DIFF_COLORS[kw.difficulty] ?? "#888", fontWeight: 600 }}>{kw.difficulty}</span></td>
                      <td style={{ padding: "6px 10px" }}><span style={{ fontSize: 11, padding: "2px 8px", borderRadius: 4, background: (INTENT_COLORS[kw.intent] ?? "#888") + "20", color: INTENT_COLORS[kw.intent] ?? "#888", fontWeight: 600 }}>{kw.intent}</span></td>
                      <td style={{ padding: "6px 10px", fontSize: 13, color: "var(--text-secondary)" }}>{kw.cpc}</td>
                      <td style={{ padding: "6px 10px", fontSize: 13, color: kw.trend === "Rising" ? "#38a169" : kw.trend === "Declining" ? "#e53e3e" : "var(--text-secondary)" }}>{kw.trend}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
