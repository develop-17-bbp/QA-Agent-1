...
export default function LocalSeo() {
  ...
  return (
    <motion.div className="qa-page" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ padding: 32 }}>
      ...
      <p className="qa-page-desc">AI-powered local SEO analysis including keyword suggestions, GBP optimization, review strategy, and citation sources.</p>
      ...
      <div className="qa-panel" style={{ padding: 16, display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <button className="qa-btn-primary" onClick={analyze} disabled={loading || !businessName.trim() || !location.trim()}>{loading ? "Analyzing..." : "Analyze"}</button>
      </div>

      {error && <div className="qa-alert qa-alert--error">{error}</div>}
      {loading && <div className="qa-loading-panel" style={{ marginTop: 20 }}><div className="qa-spinner" />Analyzing local SEO...</div>}
        ...
        <>
          ...
          {(data.localKeywords ?? []).length > 0 && (
            <div className="qa-panel" style={{ marginTop: 24, padding: 16 }}>
              <div className="qa-panel-title">Local Keywords</div>
              <table className="qa-table">
                <thead><tr>{["Keyword", "Volume", "Difficulty", "Intent", "Priority"].map(h => <th key={h}>{h}</th>)}</tr></thead>
          ...
          {(data.gbpTips ?? []).length > 0 && (
            <div className="qa-panel" style={{ marginTop: 16, padding: 16 }}>
              <div className="qa-panel-title">Google Business Profile Optimization</div>
          ...
          {data.napConsistency && (
            <div className="qa-panel" style={{ marginTop: 16, padding: 16 }}>
              <div className="qa-panel-title">NAP Consistency Score: <span style={{ color: data.napConsistency.score >= 80 ? "#38a169" : "#dd6b20" }}>{data.napConsistency.score}/100</span></div>
          ...
          {(data.listingRecommendations ?? []).length > 0 && (
            <div className="qa-panel" style={{ marginTop: 16, padding: 16 }}>
              <div className="qa-panel-title">Listing Recommendations</div>
          ...
          {(data.citationSources ?? []).length > 0 && (
            <div className="qa-panel" style={{ marginTop: 16, padding: 16 }}>
              <div className="qa-panel-title">Citation Sources</div>
              <table className="qa-table">
                <thead><tr>{["Name", "Type", "Priority"].map(h => <th key={h}>{h}</th>)}</tr></thead>
          ...
          {data.reviewStrategy && (
            <div className="qa-panel" style={{ marginTop: 16, padding: 16 }}>
              <div className="qa-panel-title">Review Strategy</div>
