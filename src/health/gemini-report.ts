 ...
 *
 * Historically this file wrapped Google's Gemini API. Gemini has been removed;
 * every LLM call now routes through the unified local-Ollama adapter in
 * `./llm.js`. The file and export names are preserved so existing imports keep
 * compiling, but the implementation is fully local.
 */
...
// ── Legacy config shims (kept so old callers compile) ───────────────────────

/**
 * Previously returned the Gemini API key. Now returns a sentinel value
 * indicating LLM generation is available (we always try local Ollama).
 * Callers that check `if (resolveGeminiApiKey())` before invoking the LLM
 * will keep working; the LLM call itself will surface a clear error if
...
/** Kept for interface parity. Always returns ["ollama"]. */
...
export const DEFAULT_GEMINI_MODEL = process.env.OLLAMA_MODEL?.trim() || "llama3.2";

/**
 * Back-compat alias for `generateText()`. Modules that still import
 * `generateGeminiText` continue to compile and now call Ollama.
 */
...
// ── Summary + Q&A generation (now Ollama-backed) ───────────────────────────
